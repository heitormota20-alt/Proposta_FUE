...
export default function Grainient({
  ...
  const applyUniforms = (program) => {
    ...
    u.uCenterOffset.value = new Float32Array([p.centerX, p.centerY])
    u.uZoom.value = p.zoom
    u.uColor1.value = new Float32Array(hexToRgb(p.color1))
    u.uColor2.value = new Float32Array(hexToRgb(p.color2))
    u.uColor3.value = new Float32Array(hexToRgb(p.color3))
    u.uLightMode.value = p.lightMode ? 1.0 : 0.0
  }

  // Cria/destrói o contexto WebGL sob demanda (só existe perto da tela), e
  // toca/pausa o loop conforme visibilidade — evita estourar o limite de
  // contextos WebGL simultâneos do navegador com muitos slides na página.
  useEffect(() => {
    ...
    const unmount = () => {
      ...
      // Libera o contexto WebGL de verdade, não só o canvas do DOM
  ...
  // Repassa mudanças de props para os uniforms sempre que já estiver montado
  useEffect(() => {
    const ctx = ctxRef.current
    if (!ctx) return
